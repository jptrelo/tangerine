...
text-shadow: 0 1px 0 rgba(255,255,255, .9);">'.$this->l('Off').'</span>
	...
	function hookhome($params){
		...
		if ($array['hook']['htmlbox_homecol']=="1"){
            $posImgTwit = strpos($array['body'], "img") - 1; //Posicion de html img
            $posFinImgTwit = strpos($array['body'], ">",$posImgTwit) + 1; //Posicion final html img
            $posImgWith = strpos($array['body'], "width", $posImgTwit); //Posicion de tamanos para despues eliminarlos
            $array['imgTwit'] = substr($array['body'], $posImgTwit, $posImgWith - $posImgTwit) . ' class="img-responsive" />'; //HTML img 
            $imgReplaceHtml = substr($array['body'], $posImgTwit, $posFinImgTwit - $posImgTwit); //HTML img para despues elminarlo del bloque html box en home
            $imgTwit = ($array['imgTwit'] == false) ? '<img src="'.$_SERVER['HTTP_REFERER'].'" alt="twitter" class="img-responsive" />' : $array['imgTwit'];
	        $array['body'] = str_replace($imgReplaceHtml,"",$array['body']);
            global $smarty;
            $smarty->assign("imgTwit", $imgTwit);
	        $smarty->assign(array('htmlboxbody' => nl2br(stripslashes($array['body'])))); 
            $smarty->assign(array('htmlboxbody' => nl2br(stripslashes($array['body'])))); 
            $smarty->assign(array( 'is_https_htmlbox' => (array_key_exists('HTTPS', $_SERVER) && $_SERVER['HTTPS'] == "on"?1:0)));
            $smarty->assign(array('htmlbox_ssl' => Configuration::get('htmlbox_ssl')));
